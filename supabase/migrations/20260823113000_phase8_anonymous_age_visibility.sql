ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS anonymous_age_visibility TEXT NOT NULL DEFAULT 'range';

ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_anonymous_age_visibility_check;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_anonymous_age_visibility_check
  CHECK (anonymous_age_visibility IN ('exact', 'range', 'hidden'));

UPDATE public.profiles
SET anonymous_age_visibility = 'range'
WHERE anonymous_age_visibility IS NULL
   OR anonymous_age_visibility NOT IN ('exact', 'range', 'hidden');
