-- Anonymous abuse identity cooldown: 15 minutes -> 5 minutes (identity-layer only).
-- Temporary suspension, moderation enforcement, and per-action rate limits unchanged.
-- Follow-up to the matchmaking incident fix: track WHEN an identity-level
-- enforcement was set, so a cooldown/suspension inherited from a previous user
-- on the same installation can be reliably distinguished and not applied.

ALTER TABLE public.anonymous_risk_identities
  ADD COLUMN IF NOT EXISTS enforcement_set_at TIMESTAMPTZ;
...
BEGIN
  ...
  ELSIF existing_row.temporary_suspension_until IS NOT NULL
    ...
    AND existing_row.enforcement_set_at >= rotation_cutoff THEN
