-- An active abuse cooldown must be a fixed deadline, not a sliding window.
--
-- Before this migration an active cooldown was only preserved while it also
-- satisfied `enforcement_set_at >= rotation_cutoff`. Whenever that stamp was
-- reset (a fresh anonymous account on the same installation moves the rotation
-- cutoff forward, and registration clears the inherited enforcement), the
-- threshold branch below it ran again and rewrote the deadline to
-- `now() + INTERVAL '5 minutes'`. A user who simply retried matching therefore
-- pushed the same cooldown forward forever instead of waiting it out.
--
-- The retrieval window is now explicit and evaluated from the persisted
-- deadline itself (each fresh window is always created as `now() + 5 minutes`,
-- so a live window always satisfies `cooldown_until > enforcement_set_at`):
--
--   * `now() < cooldown_until` and the window belongs to the current identity
--     -> keep the stored `cooldown_until` exactly; never extend it.
--   * the window has expired -> clear it and re-evaluate the risk state, so a
--     still-risky identity can open a genuinely new cooldown window.
--   * a window inherited from a previous account on the same installation is
--     still dropped, because registration clears `enforcement_set_at` (which
--     makes `cooldown_until > enforcement_set_at` fail here) before this runs.
--
-- `temporary_suspension_until` follows the same rule: an active suspension keeps
-- its stored expiry, so plain retries cannot keep renewing `now() + 24 hours`.
-- Thresholds, scoring, the 5 minute length, the 24 hour suspension, and the
-- `blocked` decision are unchanged.
BEGIN;
...
BEGIN
  ...
  ELSIF existing_row.temporary_suspension_until IS NOT NULL
    AND existing_row.enforcement_set_at IS NOT NULL
    AND existing_row.temporary_suspension_until > existing_row.enforcement_set_at
    AND timezone('utc'::text, now()) < existing_row.temporary_suspension_until THEN
    -- An active suspension keeps its stored expiry; retrying must not renew it.
  ...
  ELSIF existing_row.cooldown_until IS NOT NULL
    AND existing_row.enforcement_set_at IS NOT NULL
    AND existing_row.cooldown_until > existing_row.enforcement_set_at
    AND timezone('utc'::text, now()) < existing_row.cooldown_until THEN
    -- An active cooldown keeps its stored expiry; retrying must not renew it.
  ...
  ELSIF computed_score >= 5
    ...
    next_cooldown_until := timezone('utc'::text, now()) + INTERVAL '5 minutes';
  ELSE
    next_decision := 'allow';
  ...
  SET
    ...
    END,
    enforcement_set_at = CASE
      -- A preserved window keeps its original stamp so the deadline stays fixed.
      WHEN next_decision = 'cooldown' AND next_cooldown_until = identity_row.cooldown_until THEN identity_row.enforcement_set_at
      WHEN next_decision = 'temporary_suspension' AND next_temporary_until = identity_row.temporary_suspension_until THEN identity_row.enforcement_set_at
      WHEN next_decision IN ('cooldown', 'temporary_suspension') THEN timezone('utc'::text, now())
      WHEN next_decision = 'allow' THEN NULL
      ELSE identity_row.enforcement_set_at
    END
  WHERE identity_row.installation_key = p_installation_key;
